# Require the two stdlib classes we need, which aren't included by default
require "net/http"
require "uri"
...
module Examples
  ...
  class HTTPRequester
    ...
    def url
      # The to_s method returns the String representation of the @url variable
      @url.to_s
    end
    # Custom setter method for @url class instance variable
    # because we're parsing the url parameter before assigning to @url
    def url=(url)
      begin
        @url = URI.parse(url)
      # rescue URI::InvalidURIError exception that occurs when we try to parse an invalid URL
    ...
    # initialize method
    def initialize(url)
      # Call the setter method for the url attribute to set it to the value passed to this method
      self.url = url
...
# Let's try out our class with a valid URL Initialize our object; note that we're specifying Module::Class
...
# Here we're calling the .code method of the Net::HTTP::Request object that is returned by the get_request method
puts "Response code was #{http_requestor.get_request.code}"

# puts a blank line for spacing
puts ""

# Let's try out our class with an *invalid* URL Initialize our object; note that we're specifying Module::Class
